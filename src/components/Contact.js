...
export const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const templateParams = {
      ...
      phone: formDetails.phone, // This line already includes the phone number
      message: formDetails.message,
    };
  
    try {
      // Use EmailJS to send the email
      const response = await emailjs.sendForm(process.env.REACT_APP_EMAIL_SERVICE_ID, process.env.REACT_APP_EMAIL_TEMPLATE_ID, e.target, process.env.REACT_APP_EMAIL_USER_ID);
      ...
      setStatus({ success: false, message: "An error occurred while sending the message. Please try again later." });
    } finally {
      setButtonText("Send");
    }
  };
